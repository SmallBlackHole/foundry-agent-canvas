...
- For a matching prompt, this is a mandatory gate before file inspection or task work. If a concrete purpose is given, preserve its requirements as a concise idea: open canvas "${FOUNDRY_AGENT_CANVAS_ID}" with instanceId "${FOUNDRY_AGENT_CANVAS_INSTANCE_ID}" and input { idea }, or if already open invoke action "setAgentIdea" with input { idea }. With no concrete purpose, open it without idea, or leave it untouched if already open so Inspire me/Hello world remain available.
