 ...
 */
export async function launchAgentTerminal(session) {
    ...
    try {
        if (isOpen) {
            const recentlySent = commandSentAt && Date.now() - commandSentAt < RELAUNCH_DEBOUNCE_MS;
            if (recentlySent) {
            ...
            await session.rpc.canvas.action.invoke({
                ...
                input: { input: AGENT_RUN_COMMAND, append_newline: true },
            });
            commandSentAt = Date.now();
            logTerminal(session, "re-ran agent command in existing terminal");
        ...
        commandSentAt = Date.now();
        logTerminal(session, "opened terminal and launched agent");
